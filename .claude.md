...
- RTPSender.ReplaceTrack() - calls FFI
- Device/screen capture wired into GetUserMedia/GetDisplayMedia

## Remaining Work

### Stubs (Not Yet Implemented - Need Shim Functions)
- `PeerConnection.GetStats()` - returns empty map (needs `shim_peer_connection_get_stats`)
...
### Device Capture (WIRED UP)
Device capture is now wired into `pkg/media/media.go`:
- `CreateVideoTrack()` auto-starts camera capture when `DeviceID` is specified
- `CreateAudioTrack()` auto-starts mic capture when `DeviceID` is specified
- `GetDisplayMedia()` auto-starts screen capture when shim is loaded
- Captured frames are automatically written to tracks (discarded if not bound)
