...
func (d *vp9Decoder) init() error {
	...
	handle, err := ffi.CreateVideoDecoder(ffi.CodecVP9)
	if err != nil {
		return err
	}
...
func (d *vp9Decoder) DecodeInto(src []byte, dst *frame.VideoFrame, timestamp uint32, isKeyframe bool) error {
	if d.closed.Load() {
...
func (d *vp9Decoder) Codec() codec.Type {
	return codec.VP9
}
