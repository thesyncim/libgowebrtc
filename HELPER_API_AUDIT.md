...
- `pkg/testkit/validate` now owns its own validation-profile policy surface instead of importing browser profile types from `pkg/pioncodec`.
...
| P2 | Validation DSL and impairment tooling | `pkg/testkit/validate/doc.go`, `pkg/testkit/validate/session.go`, `pkg/testkit/validate/scenario.go`, `pkg/testkit/validate/relay.go` | Profile-gated validation session, waiters, scenario scripting, impairment relay | Powerful testkit, but very far from core media/transport API. It reads more like a testing framework than a foundational library package. | Keep it under an explicit testkit boundary rather than a runtime media package, and keep validation profiles local instead of reusing runtime/browser policy types. |
